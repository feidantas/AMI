...
ra=[];dec=[];flux=[];majs=[];mins=[];pa=[]
for row in readCSV:
	ra.append(float(row[2]))
	dec.append(float(row[4]))
	flux.append(float(row[8]))
	majs.append(float(row[14]))
	mins.append(float(row[16]))
	pa.append(float(row[18]))	
...
for i in range(0,len(ra)):  		
	...
	if (flux[i]>0.) and (dist_r<0.200):
		...
		cl.addcomponent(flux=att_flux, fluxunit='Jy', shape='gaussian', dir="J2000 "+ str(ra[i])+'deg '+str(dec[i])+'deg', majoraxis=str(majs[i])+'deg', minoraxis=str(mins[i])+'deg', positionangle=str(pa[i])+'deg') #Adjust this using the same idea if needs changing between point source and gaussian. 
